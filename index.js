...
console.log(createMenuItem('Steak', 24, 'Dinner'));

/* Task 1b: use your function to create 3 more menu items. You may add any items to the menu that you'd like */
console.log(createMenuItem('Mac and Cheese', 8, 'Side'));
console.log(createMenuItem('Whiskey', 6, 'Drink'));
console.log(createMenuItem('Ice Cream Sundae', 4, 'Dessert'));
...
const newFeedback = 'This place was great. 10/10 would eat again.';
function addRating(name, rating, feedback) {
	const newRating = reviews.push({
		name: name,
		rating: rating,
		feedback: feedback,
	});
...
console.log(addRating('Corey', 4, newFeedback));
...
for (let i = 0; i < reviews.length; i++) {
	if (reviews[i].name === target) {
		reviews[i].feedback = newReview;
	}
}
console.log(reviews);
...
function getReviewByIndex(reviews, indexNumber) {
	for (let i = indexNumber; i < reviews.length; i++) {
...
function getLastReview(reviews) {
	for (let i = reviews.length - 2; i < reviews.length; i++) {
		return `${reviews[i].name} gave the restaurant a ${reviews[i].rating} star review and their feedback was: ${reviews[i].feedback}`;
	}
}
